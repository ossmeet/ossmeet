...
export function ToastProvider({ children }: { children: ReactNode }) {
  return (
    <Toast.Provider>
      {children}
      <Toast.Portal>
        <Toast.Viewport className="fixed bottom-4 right-4 z-(--z-toast) flex w-80 max-w-[calc(100vw-2rem)] flex-col sm:bottom-4 sm:right-4 safe-bottom safe-right">
          <ToastList />
        </Toast.Viewport>
      </Toast.Portal>
    </Toast.Provider>
        ...
        <Toast.Content className="flex flex-col transition-opacity duration-[250ms] data-[behind]:pointer-events-none data-[behind]:opacity-0 data-[expanded]:opacity-100">
          ...
          <div className="h-0.5 w-full bg-neutral-100">
            <div
              ...
              style={{ width: "100%", animation: `shrink ${toast.timeout ?? 5000}ms linear forwards` }}
            />
          </div>
        </Toast.Content>
      </Toast.Root>
