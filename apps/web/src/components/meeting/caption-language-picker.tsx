  ...
  return (
    <div className={cn("space-y-2", className)}>
      <div className={cn(
        ...
        <Search className={cn("h-4 w-4", isLight ? "text-stone-400" : "text-white/45")} />
        <input
          ref={searchInputRef}
          type="search"
          value={query}
          ...
          return (
            ...
            >
              ...
              <span className="flex shrink-0 items-center gap-2">
                {active && <Check className={cn("h-4 w-4", isLight ? "text-teal-600" : "text-primary-300")} />}
                <span className={cn("font-mono text-2xs", isLight ? "text-stone-400" : "text-white/35")}>{option.tag}</span>
              </span>
            </button>
          );
        })}
        {options.length === 0 && (
          <div className={cn("px-3 py-2 text-sm", isLight ? "text-stone-500" : "text-white/45")}>
