    ...
    <div className="flex h-full flex-col gap-3 overflow-y-auto p-3">
      {activeTracks.map((track) => {
        ...
        return (
          ...
          >
            <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-stone-900 shadow-[0_2px_8px_-2px_rgba(0,0,0,0.15)] transition-all hover:shadow-[0_4px_12px_-2px_rgba(0,0,0,0.2)]">
              <ParticipantTile
                trackRef={track}
                className={cn(
                  "h-full w-full object-cover [&_video]:object-cover [&_video]:w-full [&_video]:h-full",
              ...
              {/* Camera switch button - only for local video, visible when multiple cameras */}
              {isLocal && track.source === Track.Source.Camera && onSelectVideoDevice && videoDevices.length > 1 && (
                <div className="absolute right-2 top-2 z-10 opacity-70 transition-opacity duration-200 hover:opacity-100 pointer-events-auto">
                  <CameraSwitchButton
                    videoDevices={videoDevices}
                    currentDeviceId={currentVideoDeviceId}
                    onSelectDevice={onSelectVideoDevice}
            ...
            <div className="flex items-center justify-between gap-1.5 px-0.5">
              <span className="truncate text-xs font-medium text-stone-600">
                {participantName}
                {track.participant.isLocal && <span className="font-normal text-stone-400"> (You)</span>}
              </span>
            </div>
          </div>
        );
