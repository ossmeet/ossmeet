...
import { getCameraCaptureDefaults } from "@/lib/meeting/media-quality";
import {
  pickNativeAudioDevice,
  pickNativeVideoDevice,
} from "@/lib/meeting/device-selection";
import { warmUpSpeechRecognition } from "@/lib/meeting/speech-warmup";
...
// ── Audio warmup stream ───────────────────────────────────────────────────────
// Keeps a silent getUserMedia audio stream alive from device enumeration until
// LiveKit acquires the mic. Without this, LiveKit's cold-start audio request
// triggers macOS Continuity Camera to briefly activate a nearby iPhone.
// Module-level so it survives PreJoinScreen unmounting (which happens before
// LiveKit opens its own stream).

let _warmupAudioStream: MediaStream | null = null;
let _warmupCallId = 0;

function startWarmupAudio(deviceId: string) {
  stopWarmupAudio();
  const callId = ++_warmupCallId;
  navigator.mediaDevices
    .getUserMedia({ audio: { deviceId: { exact: deviceId } } })
    .then((stream) => {
      if (callId !== _warmupCallId) {
        stream.getTracks().forEach((t) => t.stop());
        return;
      }
      _warmupAudioStream = stream;
    })
    .catch(() => {
      // Best-effort — failing silently is fine.
    });
}

export function stopWarmupAudio() {
  ++_warmupCallId;
  if (!_warmupAudioStream) return;
  _warmupAudioStream.getTracks().forEach((t) => t.stop());
  _warmupAudioStream = null;
}
...
export function PreJoinScreen({ meetingTitle, waitingForHost = false, noActiveSession = false, user, onJoin }: PreJoinScreenProps) {
  ...
  const [selectedAudioDevice, setSelectedAudioDevice] = React.useState<
    string | undefined
  >(undefined);
  const userSelectedVideoDeviceRef = React.useRef(false);
  const userSelectedAudioDeviceRef = React.useRef(false);
  ...
  const [captionLanguage, setCaptionLanguage] = React.useState(() =>
    speechSupported
      ? pickSpeechLanguage({
          savedLanguage: loadSavedSpokenLanguage(),
  ...
  const initialBgPreference = React.useMemo(() => {
    const saved = loadBackgroundPreference();
    return { mode: "none" as BackgroundMode, imagePath: saved.imagePath };
  }, []);
  ...
  const getDevices = React.useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        setVideoDevices([]);
        setAudioDevices([]);
        setDeviceError("Media device APIs are not available in this browser.");
        return;
      }
      ...
      let audioPermissionDenied = false;
      ...
      // Request audio without an exact deviceId — before audio permission is
      // granted the browser returns placeholder entries with deviceId:"", and
      // using {exact:""} would cause an immediate NotFoundError.
      try {
      ...
      // Filter out placeholder entries browsers emit before permission is granted
      // (deviceId:"" means the browser hasn't exposed the real ID yet).
      const freshVideo = devices.filter((d) => d.kind === "videoinput" && d.deviceId !== "");
      const freshAudio = devices.filter((d) => d.kind === "audioinput" && d.deviceId !== "");
      ...
      if (videoProbeFailed && audioProbeFailed && !videoGranted && !audioGranted) {
        setVideoEnabled(false);
        setAudioEnabled(false);
        setDeviceError(
          !hasCamera && !hasMicrophone
            ? NO_MEDIA_DEVICES_MESSAGE
        ...
        setVideoEnabled(false);
        setDeviceError(
          videoPermissionDenied || hasCamera
        ...
        setAudioEnabled(false);
        setDeviceError(
          audioPermissionDenied || hasMicrophone
  ...
  React.useEffect(() => {
    ...
    const selectedStillExists =
      !!selectedVideoDevice &&
      videoDevices.some((device) => device.deviceId === selectedVideoDevice);
    if (userSelectedVideoDeviceRef.current && selectedStillExists) return;
    if (!selectedStillExists) userSelectedVideoDeviceRef.current = false;
    const best = pickNativeVideoDevice(videoDevices);
    if (best && selectedVideoDevice !== best.deviceId) {
  ...
  React.useEffect(() => {
    ...
    const selectedStillExists =
      !!selectedAudioDevice &&
      audioDevices.some((device) => device.deviceId === selectedAudioDevice);
    if (userSelectedAudioDeviceRef.current && selectedStillExists) return;
    if (!selectedStillExists) userSelectedAudioDeviceRef.current = false;
    const best = pickNativeAudioDevice(audioDevices);
    if (best && selectedAudioDevice !== best.deviceId) {
      setSelectedAudioDevice(best.deviceId);
    }
  }, [selectedAudioDevice, audioDevices]);

  // Start warmup audio when a valid device is selected. Keeps the mic "warm"
  // so LiveKit's stream open on join is not a cold start (which triggers macOS
  // Continuity Camera to briefly activate a nearby iPhone).
  // No cleanup return: the warmup must survive PreJoinScreen unmounting because
  // LiveKit doesn't open its stream until after the join API call completes.
  // stopWarmupAudio() is called from handleRoomConnected in the parent.
  React.useEffect(() => {
    if (!audioEnabled) {
      stopWarmupAudio();
      return;
    }
    if (!selectedAudioDevice || audioDevices.length === 0 || isLoadingDevices) return;
    startWarmupAudio(selectedAudioDevice);
    warmUpSpeechRecognition();
  }, [audioEnabled, selectedAudioDevice, audioDevices.length, isLoadingDevices]);
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex h-10 w-full items-center rounded-lg border border-stone-200 bg-stone-50 px-3">
                    ...
                    <>
                      <input
                        ...
                        className={cn(
                          "h-10 w-full rounded-lg border bg-white px-3 text-sm text-stone-900 placeholder-stone-400 transition-all focus-visible:outline-hidden focus-visible:ring-2",
                          nameError
                            ? "border-red-400 focus-visible:border-red-400 focus-visible:ring-red-400/20"
                            : "border-stone-200 focus-visible:border-teal-500 focus-visible:ring-teal-500/20"
                        )}
                      />
                      {nameError && (
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      userSelectedVideoDeviceRef.current = true;
                      setSelectedVideoDevice(v ?? undefined);
                    }}
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      userSelectedAudioDeviceRef.current = true;
                      setSelectedAudioDevice(v ?? undefined);
                    }}
                ...
                {speechSupported && (
                  <div ref={captionLanguagePickerRef} className="relative">
                    ...
                    <button
                      ...
                      className="flex h-10 w-full items-center justify-between rounded-lg border border-stone-200 bg-white px-3 text-left text-sm text-stone-800 transition-all hover:border-stone-300 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-teal-500/20"
                    >
