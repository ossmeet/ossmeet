  ...
  const createMutation = useMutation({
    ...
    onSuccess: (result) => {
      rememberCreatedMeetingLookup(result.code, {
        ...
        locked: false,
        hostName: sessionData?.user?.name ?? null,
        requiresAuth: !allowGuests,
      });
      ...
      <DialogContent size="sm">
        ...
        <div className="px-6 py-4 space-y-4">
          <input
            ...
            maxLength={200}
            className="w-full text-sm text-neutral-900 placeholder:text-neutral-400 bg-stone-50 border border-stone-200 rounded-xl px-3 py-2.5 outline-hidden focus:ring-2 focus:ring-accent-500/30 focus:border-accent-400"
          />
                  ...
                  {useCustomCode && (
                    <div>
                      <input
                        ...
                        autoCapitalize="none"
                        spellCheck={false}
                        className="h-9 w-full px-3 rounded-lg border border-neutral-300 text-sm font-mono tracking-wider transition-all focus:outline-hidden focus:ring-2 focus:ring-accent-500/30 focus:border-accent-500"
                        maxLength={12}
                      />
                      {customCodeError && (
