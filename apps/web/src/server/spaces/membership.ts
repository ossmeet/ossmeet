...
export const addMember = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context: { user, env, db } }) => {
    ...
    const now = new Date();
    const insertResult = await withD1Retry(() =>
    ...
    await withD1Retry(() =>
      db.update(spaces).set({ updatedAt: now }).where(eq(spaces.id, data.spaceId)),
    );
...
export const addMemberByEmail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context: { user, env, db } }) => {
    ...
    if (changes > 0) {
      await withD1Retry(() =>
        db.update(spaces).set({ updatedAt: new Date() }).where(eq(spaces.id, data.spaceId)),
      );
    }
