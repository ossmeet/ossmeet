...
import { sessions, meetingAdmissions, devices, users } from "@ossmeet/db/schema";
...
export async function getEnv(): Promise<Env> {
  ...
  try {
    ...
    const { env: cfEnv } = await import(/* @vite-ignore */ cloudflareWorkersModule);
    return cfEnv as Env;
  } catch {
...
/**
 * Return the Cloudflare ExecutionContext for the current request, if any.
 * Used to defer work via ctx.waitUntil() after the response has been sent.
 */
export function getCloudflareCtx(): ExecutionContext | undefined {
  try {
    const request = getRequest();
    return (request as unknown as { __cloudflare?: { ctx?: ExecutionContext } }).__cloudflare?.ctx;
  } catch {
    return undefined;
  }
}
...
export function getGuestCookieSecretFromCookie(
  ...
  return getCookieValue(cookie, guestCookieName(admissionId));
}
...
function rateLimitLogPayload(key: string, env: Env) {
  const keyScope = key.split(":")[0];
  // Truncated hash for log correlation without leaking PII
  let hash = 0;
  for (let i = 0; i < key.length; i++) {
    hash = ((hash << 5) - hash + key.charCodeAt(i)) | 0;
  }
  const keyHash = Math.abs(hash).toString(16).padStart(12, "0").slice(0, 12);
  return { keyScope, keyHash, environment: env.ENVIRONMENT };
}
...
export async function enforceRateLimit(
  ...
  if (env.ENVIRONMENT === "production" || env.ALLOW_MEMORY_RATE_LIMIT !== "true") {
    logError(
      "[RateLimit] Cloudflare binding unavailable and memory fallback is disabled; rejecting request.",
      rateLimitLogPayload(key, env),
    );
    throw Errors.CONFIG_ERROR("Rate limiter binding not configured");
  }

  logWarn(
    "[RateLimit] Cloudflare binding not configured; using in-memory fallback.",
    rateLimitLogPayload(key, env),
  );
...
async function enforceDeviceCap(db: Database, userId: string): Promise<void> {
  ...
  if (userDevices.length > MAX_DEVICES_PER_USER) {
    ...
    await withD1Retry(() =>
      db.delete(devices).where(inArray(devices.id, toDelete)),
    );
  }
}
...
/** Cookie name for a guest admission's secret. */
export function guestCookieName(admissionId: string): string {
  return `ossmeet_guest_${admissionId}`;
}
 ...
 */
export function getGuestCookieSecret(admissionId: string): string | null {
  try {
    const request = getRequest();
    return getGuestCookieSecretFromCookie(request.headers.get("Cookie"), admissionId);
  } catch {
    return null;
  }
}
...
/**
 * Verify that the caller owns a guest admission row.
 * Reads the secret from the HttpOnly cookie set at join time, looks up the
 * admission, and does a timing-safe comparison against the stored hash.
 * Throws UNAUTHORIZED if the cookie is absent, FORBIDDEN if the admission
 * is not found or the secret does not match.
 * Returns the full admission row on success.
 */
export async function verifyGuestAdmission(
  db: Database,
  meetingId: string,
  admissionId: string,
) {
  const guestSecret = getGuestCookieSecret(admissionId);
  return verifyGuestAdmissionBySecret(db, meetingId, admissionId, guestSecret);
}

export async function verifyGuestAdmissionBySecret(
  db: Database,
  meetingId: string,
  admissionId: string,
  ...
  if (!await verifyGuestSecret(admission.guestSecretHash, guestSecret)) throw Errors.FORBIDDEN();
...
export async function enforceSessionCap(db: Database, userId: string): Promise<void> {
  ...
  if (userSessions.length > MAX_SESSIONS) {
    ...
    await withD1Retry(() =>
      db.delete(sessions).where(inArray(sessions.id, idsToDelete)),
    );
  }
}
