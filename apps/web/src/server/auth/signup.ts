...
export async function consumeVerificationById(
  db: Database,
  verificationId: string,
): Promise<boolean> {
  const result = await withD1Retry(() =>
    db.delete(verifications).where(eq(verifications.id, verificationId)).run(),
  );
...
export const verifyOTP = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!(await consumeVerificationById(db, existingVerification.id))) {
      throw createValidationError("Invalid or expired OTP");
    }
