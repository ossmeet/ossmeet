...
import { consumeVerificationById, verifyOtpWithAttempts } from "./signup";
  ...
  .handler(async ({ context: { user: currentUser, env, db } }) => {
    ...
    if (toDelete.length > 0) {
      for (const chunk of chunkArray(toDelete, d1MaxItemsPerStatement())) {
        await withD1Retry(() =>
          db.delete(sessions).where(inArray(sessions.id, chunk)),
        );
      }
    }
...
export const requestAccountDeletion = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context: { user: currentUser, env, db } }) => {
    ...
    const changes = getRunChanges(insertResult);
    if (changes === 0) {
...
export const deleteAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context: { user: currentUser, env, db } }) => {
    ...
    if (!(await consumeVerificationById(db, verification.id))) {
      throw Errors.UNAUTHORIZED();
    }
    ...
    if (ownedSpaces.length > 0) {
      throw Errors.VALIDATION(
        `You own ${ownedSpaces.length} space(s). Please delete or transfer ownership before deleting your account.`
      );
    }
    ...
    if (userRooms.length > 0) {
      ...
      // Defensive: the active-meeting check above should make this empty,
      // but cover any race where a session became active in the interim.
      const activeRoomSessions: { id: string }[] = [];
      ...
      if (activeRoomSessions.length > 0) {
        await Promise.allSettled(
          activeRoomSessions.map((meeting) => endSession(db, env, meeting.id, "system")),
        );
      }

      for (const chunk of chunkArray(roomIds, 100)) {
        await withD1Retry(() =>
          db.delete(rooms).where(inArray(rooms.id, chunk)),
        );
      }
    }
    ...
    try {
      ...
      if (userAssets.length > 0) {
        ...
        await withD1Retry(() =>
          db.delete(meetingArtifacts).where(eq(meetingArtifacts.uploadedById, currentUser.id)),
        );
        await withD1Retry(() =>
          db.delete(spaceAssets).where(eq(spaceAssets.uploadedById, currentUser.id)),
        );
      }
