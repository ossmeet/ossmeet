 ...
 */
export const getGoogleLinkUrl = createServerFn({ method: "POST" }).handler(
  async () => {
    const env = await getEnv();
    const db = createDb(env.DB);
    await requireAuth();
...
export const linkGoogleAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const conflictedAccount = await db.query.accounts.findFirst({
        where: and(
          eq(accounts.providerId, "google"),
          eq(accounts.providerAccountId, payload.sub),
        ),
      });
      if (conflictedAccount?.userId === currentUser.id) {
        return { success: true, alreadyLinked: true };
      }
      throw Errors.VALIDATION("This Google account is already linked to another user.");
    }
