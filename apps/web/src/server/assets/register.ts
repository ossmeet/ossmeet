...
export async function registerMeetingArtifactMetadata(
  ...
  const overwriteOnConflict = input.overwriteOnConflict ?? true;
  ...
  const inserted = await withD1Retry(() => {
    ...
    const statement = overwriteOnConflict
      ? baseInsert.onConflictDoUpdate({
          target: meetingArtifacts.r2Key,
          set: {
            sessionId,
            spaceId: input.spaceId,
            type: input.type,
    ...
    return statement.returning({ id: meetingArtifacts.id });
  });
