...
export const Route = createLazyFileRoute("/_authed/meetings/$code")({
  component: MeetingRecapPage,
  pendingComponent: () => (
    <div className="flex h-64 items-center justify-center">
      <Spinner size="lg" brand />
    </div>
  ),
  errorComponent: MeetingRecapError,
});

function MeetingRecapError({ error }: { error: Error }) {
...
function MeetingRecapPage() {
  ...
  const {
    summaryData: recapData,
    summaryError: recapError,
    existingSummary,
    generationError,
    isGenerating,
    isPreparingSummary,
    ...
    return (
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <EmptyState
          icon={AlertTriangle}
          title="Could not load meeting recap"
          description="We couldn't load the recap for this meeting link. Please try again."
          action={
            <Button
              variant="secondary"
              size="sm"
  ...
  if (!recapData?.session) {
    return (
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        ...
        <EmptyState
          icon={Clock}
          title="No ended meetings yet"
          description="This meeting link has no recap history yet."
        />
      </div>
    );
  }
