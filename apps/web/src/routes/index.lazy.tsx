...
import { getErrorMessage, getServerErrorCode, getServerErrorStatusCode } from "@/lib/errors";
import { beginMeetingEntryFlow, rememberCreatedMeetingLookup } from "@/lib/meeting/entry-metrics";
...
const CREATE_ERROR_FALLBACK = "Failed to create meeting. Please try again.";

function getCreateErrorMessage(error: unknown): string {
  switch (getServerErrorCode(error)) {
    case "PLAN_LIMIT_REACHED":
      return "You've reached your plan's concurrent meeting limit.";
    case "FORBIDDEN":
      return "Your plan doesn't allow this meeting type.";
    case "RATE_LIMITED":
      return "Too many attempts. Please wait a moment and try again.";
    case "CONFLICT":
      return "That meeting code is already taken.";
    case "VALIDATION":
      return getErrorMessage(error, CREATE_ERROR_FALLBACK);
    default:
      return getErrorMessage(error, CREATE_ERROR_FALLBACK);
  }
}
...
function LandingPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (result) => {
      rememberCreatedMeetingLookup(result.code, {
        exists: true,
        kind: result.permanent ? "permanent" : "instant",
        title: null,
    ...
    onError: async (error) => {
      ...
      const statusCode = getServerErrorStatusCode(error);
      ...
      setCreateErrorMessage(getCreateErrorMessage(error));
    },
  });

  const { mutate: createMeetingMutate, isPending: createPending } =
    createMutation;

  // Button is hidden until a session is loaded, so `session.user` is always
  // defined here. Keeping the typed guard for safety, no async fallback.
  const handleCreateMeeting = useCallback(() => {
    if (!session?.user) return;
    setCreateErrorMessage("");
    createMeetingMutate(getPlanLimits(session.user.plan).recordingEnabled);
  }, [createMeetingMutate, session]);
