  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        let body: string;
        try {
          body = await readRequestBodyText(request, MAX_PADDLE_WEBHOOK_BODY_BYTES);
        } catch (err) {
        ...
        let event: { event_type: string; occurred_at?: string; data: Record<string, unknown> };
        try {
...
async function handlePaddleEvent(
  event: { event_type: string; occurred_at?: string; data: Record<string, unknown> },
  env: Env,
): Promise<void> {
  const { event_type, occurred_at: occurredAt, data } = event;
  ...
  if (
    ...
    const isEntitled = subscriptionStatus === "active" || subscriptionStatus === "trialing";
    const resolvedPlan = isEntitled && plan ? plan : isEntitled ? undefined : "free" as PlanType;

    const db = createDb(env.DB);
    const userRow = await db.query.users.findFirst({
      where: eq(users.paddleCustomerId, customerId),
      columns: { id: true, subscriptionUpdatedAt: true },
    });
    if (!userRow) {
    ...
    if (userRow.subscriptionUpdatedAt && eventTime < new Date(userRow.subscriptionUpdatedAt)) {
      logWarn("[paddle-webhook] Stale event ignored", {
        event_type,
        occurredAt,
        subscriptionUpdatedAt: userRow.subscriptionUpdatedAt,
      });
      return;
    }
    ...
    const result = await withD1Retry(() =>
      db.update(users).set({
        ...
        subscriptionUpdatedAt: eventTime,
        updatedAt: appliedAt,
      }).where(
        and(
          eq(users.id, userRow.id),
          or(isNull(users.subscriptionUpdatedAt), lte(users.subscriptionUpdatedAt, eventTime)),
        ),
      ).run()
    );
    if (getRunChanges(result) === 0) {
      logWarn("[paddle-webhook] Concurrent stale event ignored", { event_type, occurredAt });
    }
    return;
  }
