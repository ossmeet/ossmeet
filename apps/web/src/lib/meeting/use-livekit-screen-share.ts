...
export function useLiveKitScreenShare(
  ...
  // If the original host leaves while a participant is waiting, the request
  // message was already received before the successor became admin. Re-send
  // the pending request when a host/promoted host appears so the request tray
  // is populated without asking the participant to click again.
  useEffect(() => {
    if (!room || isAdmin || requestState !== "pending" || !userId || !userName) return;

    const resendRequest = () => {
      publishMessage({ type: "screen_share.request", userId, userName });
    };
    const handleParticipantMaybeAdmin = (participant: RemoteParticipant | LocalParticipant) => {
      if (hasAdminGrant(participant)) resendRequest();
    };
    const handleMetadataChanged = (_prev: string | undefined, participant: RemoteParticipant | LocalParticipant) => {
      handleParticipantMaybeAdmin(participant);
    };
    const handleParticipantConnected = (participant: RemoteParticipant) => {
      handleParticipantMaybeAdmin(participant);
    };

    if ([...room.remoteParticipants.values()].some(hasAdminGrant)) {
      resendRequest();
    }
  ...
  }, [room, isAdmin, requestState, userId, userName, publishMessage]);
