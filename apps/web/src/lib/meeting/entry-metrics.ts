...
type EntryMetricsState = Partial<Record<EntryMetricKey, number>> & {
  ...
  flowId?: string;
  loggedStages?: string[];
  createdMeetingLookup?: CreatedMeetingLookup;
};

export type CreatedMeetingLookup = {
  exists: true;
...
export function rememberCreatedMeetingLookup(
  code: string,
  lookup: CreatedMeetingLookup,
) {
  const state = getState();
  state.code = code;
  state.createdMeetingLookup = lookup;
  writeState(state);
}

export function readCreatedMeetingLookup(code: string): CreatedMeetingLookup | null {
  const state = readState();
  if (!state || state.code !== code) return null;
  return state.createdMeetingLookup ?? null;
}
