...
export function buildCheckoutOpenOptions({
  ...
  customerId,
  email,
}: OpenCheckoutOptions): PaddleCheckoutOpenOptions {
  return {
    settings: {
      displayMode: "overlay",
    },
    items: [{ priceId, quantity: 1 }],
    customer: email ? { id: customerId, email } : { id: customerId },
  };
}
...
export async function openPaddleCheckout({
  ...
  paddle.Checkout.open(buildCheckoutOpenOptions({ priceId, customerId, email }));
}
