...
function buildProductionCsp(config?: {
  appUrl?: string;
  livekitUrl?: string;
  addonCspDomains?: string[];
}): string {
  const addonDomains = config?.addonCspDomains ?? [];
  ...
  return [
    ...
    // static.cloudflareinsights.com: Cloudflare Web Analytics beacon injected at the edge.
    "script-src 'self' 'unsafe-inline' https://cdn.paddle.com https://static.cloudflareinsights.com",
...
export function buildMeetRouteCsp(config?: {
  appUrl?: string;
  livekitUrl?: string;
  addonCspDomains?: string[];
}): string {
  ...
  return [
    ...
    [
      ...
      `https://${livekitDomain}`,
      `wss://${livekitDomain}`,
      `https://${livekitTurnDomain}`,
      `wss://${livekitTurnDomain}`,
      ...addonDomains.flatMap((d) => [`https://${d}`, `wss://${d}`]),
    ].join(" "),
