...
const MAX_WHITEBOARD_JWT_TTL_SECONDS = 24 * 3600;
const MAX_WHITEBOARD_JWT_CLOCK_SKEW_SECONDS = 60;
...
export async function verifyWhiteboardJWT(
  ...
  if (
    ...
    typeof (claims as WhiteboardJWTClaims).connectionId !== "string" ||
    ("service" in (claims as Record<string, unknown>) &&
      typeof (claims as WhiteboardJWTClaims).service !== "string") ||
    typeof (claims as WhiteboardJWTClaims).exp !== "number" ||
    typeof (claims as WhiteboardJWTClaims).iat !== "number" ||
    ...
    throw new Error("JWT missing required claims (sub, name, role, sid, exp, iat, iss, aud)");
  }

  const validated = claims as WhiteboardJWTClaims;

  // Sanitize name: reject control characters, enforce max length
  if (validated.name.length > 256 || /[\x00-\x1f\x7f]/.test(validated.name)) {
    throw new Error("JWT name claim contains invalid characters or is too long");
  }
  ...
  if (validated.iat > now + MAX_WHITEBOARD_JWT_CLOCK_SKEW_SECONDS) {
    throw new Error("JWT issued-at is in the future");
  }
  if (validated.exp - validated.iat > MAX_WHITEBOARD_JWT_TTL_SECONDS) {
