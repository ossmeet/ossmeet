...
import type { WikiArticle, WikiImage } from "@/lib/meeting/wiki-types";
...
async function searchWikipedia(query: string, whiteboardToken?: string | null): Promise<string[]> {
  const url = new URL("/api/wiki", window.location.origin);
  url.searchParams.set("type", "search");
  url.searchParams.set("q", query);

  const res = await fetch(url.toString(), {
    headers: whiteboardToken ? { Authorization: `Bearer ${whiteboardToken}` } : undefined,
  });
...
async function getArticle(title: string, whiteboardToken?: string | null): Promise<WikiArticle | null> {
...
export function WikiSearchPanel({
  ...
  const runSearch = React.useCallback(async (query: string, broadcast = true) => {
    ...
    try {
      const titles = await searchWikipedia(query, whiteboardToken);
      if (!titles.length) {
        setState({ type: "no_results", query });
        return;
      }
      const article = await getArticle(titles[0], whiteboardToken);
      if (!article) {
        setState({ type: "no_results", query });
        return;
      }
  ...
  }, [onBroadcastSearch, userName, whiteboardToken]);
