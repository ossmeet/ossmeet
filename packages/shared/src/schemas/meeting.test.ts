...
describe('toggleStreamingSchema', () => {
  it('accepts built-in streaming platforms', () => {
    expect(
      toggleStreamingSchema.safeParse({
        sessionId: 'msn_1',
        action: 'start',
        platform: 'kick',
        streamKey: 'live_123',
  ...
  it('accepts custom RTMP destinations', () => {
    expect(
      toggleStreamingSchema.safeParse({
        sessionId: 'msn_1',
        action: 'start',
        platform: 'instagram',
        streamKey: 'rtmps://stream.example.com/app/live_123',
      }).success,
    ).toBe(true)
  })

  it('rejects unknown streaming platforms', () => {
    expect(
      toggleStreamingSchema.safeParse({
        sessionId: 'msn_1',
        action: 'start',
