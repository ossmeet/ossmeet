...
export const joinMeetingSchema = z.object({
  ...
  // Allow reconnect via admissionId — guest secret is read from the HttpOnly cookie.
  reconnectAdmissionId: z.string().min(1).optional(),
});
...
export const toggleStreamingSchema = z.object({
  sessionId: z.string().min(1),
  action: z.enum(["start", "stop"]),
  platform: z.enum(STREAMING_PLATFORMS).optional(),
  streamKey: z.string().min(1).max(2048).optional(),
  egressId: z.string().optional(),
});

export const meetingLivekitPresencesListSchema = z.object({
  sessionId: z.string().min(1),
});
